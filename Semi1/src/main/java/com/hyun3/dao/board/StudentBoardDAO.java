...
public class StudentBoardDAO {
	...
	// 게시글 리스트 가져오기
	public List<StudentBoardDTO> listBoard(String division, int offset, int size, String category, String schType,
			String kwd) {
		List<StudentBoardDTO> list = new ArrayList<>();
		StringBuilder sql = new StringBuilder();
	...
	public StudentBoardDTO findByNum(long cmNum) {
		String sql = "SELECT s.CM_NUM, s.DIVISION, s.TITLE, s.CONTENT, s.VIEWS, s.CA_DATE, s.FILENAME, s.MB_NUM,"
				+ "m.USERID, m.NICKNAME, c.CT_NUM, c.CT_NAME " + "FROM STUDENTBOARD s "
				+ "JOIN MEMBER m ON s.MB_NUM = m.MB_NUM " + "LEFT JOIN CATEGORY c ON s.CT_NUM = c.CT_NUM "
				+ "WHERE s.CM_NUM = ?";
	...
	public StudentBoardDTO findByPrev(String division, long cmNum, String category, String schType, String kwd) {
		...
		sb.append("SELECT s.CM_NUM, s.DIVISION, s.TITLE, s.CONTENT, s.VIEWS, s.CA_DATE, s.FILENAME, ")
				.append("m.USERID, m.NICKNAME, c.CT_NUM, c.CT_NAME ").append("FROM STUDENTBOARD s ")
				.append("JOIN MEMBER m ON s.MB_NUM = m.MB_NUM ").append("LEFT JOIN CATEGORY c ON s.CT_NUM = c.CT_NUM ")
	...
	public int dataCount(String division, String category, String schType, String kwd) {
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT COUNT(*) AS cnt FROM STUDENTBOARD s ").append("WHERE s.DIVISION = ? ");
		...
		if (kwd != null && !kwd.isEmpty()) {
			if ("all".equals(schType)) {
				...
				sql.append(
						" AND EXISTS (SELECT 1 FROM MEMBER m WHERE m.MB_NUM = s.MB_NUM AND INSTR(m.NICKNAME, ?) >= 1) ");
			} else {
